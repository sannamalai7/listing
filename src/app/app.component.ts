import { Component, ElementRef, Inject, HostListener, ChangeDetectorRef, ViewChild } from '@angular/core';
...
export class AppComponent {
	...
	constructor(private element: ElementRef, private ref: ChangeDetectorRef, private pagerService: PagerService) {	
		...
		if (this.inputUrl != '' && this.inputUrl != ' ') { 
			...
			{
				this.url = this.url+"&qterm="+this.getParameterByName('qterm', document.location.search);
			}
			...
			{
				this.url = this.url+"&srt="+this.getParameterByName('srt', document.location.search);
			}
	...
	ngOnInit() {
        ...
        this.leftArrow = localeVal[1] == 'ar' ? 'fa fa-angle-right' : 'fa fa-angle-left';
		this.rightArrow = localeVal[1] == 'ar' ? 'fa fa-angle-left' : 'fa fa-angle-right';
	}
	
	ngAfterViewInit() {
		if(this.isMobile) {
			const threshold = 0.3; // how much % of the element is in view
			let rowsCount = this.rows + Number(this.getParameterByName('rows', this.url));
	...
	public updateParameter(parameter: any) {	
       ...
       if(this.qterm!=undefined && this.qterm!=""){
		...
		if(len_exact_val[1]==undefined || len_exact_val[1].length<1) {
			...
			this.url = this.removeURLParameter(this.url, 'lang_exact');
			this.url = this.removeURLParameter(this.url, 'apilang');
			this.url = this.url+'&apilang='+langVal+'&lang_exact='+len_exact_val[1];
		}
		//
