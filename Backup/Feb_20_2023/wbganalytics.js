...
function analyticParams(localurl, sterm, sresults, sortby) {
  console.log("analyticParams call");
  ...
  if (query_string) {
    var queryPair = query_string.split("&");
    ...
    for (var i = 0; i < queryPair.length; i++) {
      ...
      if (
        typeof paramMap[queryParamName] != "undefined" &&
        queryParamName != "pastevents" &&
        queryParamName != "futureevents" &&
        queryParamName != "srt" &&
        queryParamName != "qterm" &&
        queryParamName != "x" &&
        queryParamName != "y"
      ) {
